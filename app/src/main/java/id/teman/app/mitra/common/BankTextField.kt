...
@Composable
fun CategoriesRestaurantFormField(value: String = "", onSelected: (CategoriesRestaurantSpec) -> Unit, listBank: List<CategoriesRestaurantSpec>) {
    var expanded by remember { mutableStateOf(false) }
    val icon = if (expanded) Icons.Filled.KeyboardArrowUp else Icons.Filled.KeyboardArrowDown
    var textFiledSize by remember { mutableStateOf(Size.Zero) }
    Column(
        ...
        Text("Pilih Kategori Restoran", style = UiFont.poppinsP2Medium)
        Spacer(modifier = Modifier.height(Theme.dimension.size_8dp))
        OutlinedTextField(value = value,
            textStyle = UiFont.poppinsP2Medium.copy(color = UiColor.neutral900),
            enabled = false,
            placeholder = {
                Text("Pilih Kategori", style = UiFont.poppinsP2Medium.copy(color = UiColor.neutral500))
            },
            shape = RoundedCornerShape(Theme.dimension.size_4dp),
            onValueChange = {
                // no op
